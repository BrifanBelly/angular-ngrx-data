...
**Dispatcher server methods return Observables**
...
In this release, **these methods return an Observable of the operation result** which emits when the
server responds and after the reducers have applied the result to the collection.
Now you can subscribe to these observables.
...
Your existing service callers will compile and work as before.
...
These overrides must now return an appropriate Observable.
